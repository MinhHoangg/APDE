    ...
    <style>
      ...
      <body align = "center" background="img/pic.png">
        ...
        <ul>
          ...
          <form class="w3-container" name="Insert" action="assignPHP.php" method="POST">
            <p>
              <label>Name: </label>
              <input class="w3-input" type="text" name="name" required></p>
              <p>
                <label>Topic: </label>
                <select name="topicName">
                  ...
                  $sql = "SELECT DISTINCT topicName FROM topiclist";
                  $row = mysqli_query($conn, $sql);
                  foreach ($row as $r) 
                  {
                    ?>
                    <option value="<?=$r['topicName']?>"><?=$r['topicName']?></option>
                    <?php
                  }
                  ?>
                </select></p>
                <p>
                  ...
                  <select name="courseName">
                    ...
                    {
                      ?>
                      <option value="<?=$r['courseName']?>"><?=$r['courseName']?></option>
                      <?php
                    }
                    ?>
                  </select></p>
                  <p>
                    ...
                    <select name="trainerName">
                      ...
                      $conn = new mysqli($servername, $username, $password, $dbname);
                      $sql = "SELECT DISTINCT trainerName FROM trainerList";
                      $row = mysqli_query($conn, $sql);
                      foreach ($row as $r) 
                      {
                        ?>
                        <option value="<?=$r['trainerName']?>"><?=$r['trainerName']?></option>
                        <?php
                      }
                      ?>
                    </select></p>
                ...
                <table align="center" id="customers">
                  <tr>
                    <th align='center' width='200'>ID</th><th align='center' width='200'>Trainee Name</th>
                    <th align='center' width='200'>Topic</th>
                    <th align='center' width='200'>Course</th>
                    <th align='center' width='200'>Trainer Name</th></tr>
                  </table>
                  <?php
                  $servername = "localhost";
                  $username = "root";
